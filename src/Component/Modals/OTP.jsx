...
const OTP = (props) => {
  ...
  const OtpVerify = async (otp) => {
    if (otp.length === 4) {
      setLoader(true);
      const res = await SubmitResponse(`${LoginbaseURL}/verifyOtp`, {
        otp: otp,
        ...props?.formValue,
      });
      if (res?.data?.status == 200) {
  ...
  return (
    <>
      <Modal {...props} centered backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          <div className="modal_sec_in">
            ...
            <p>
              We've emailed a 4-digit code. Please find it in your inbox (or
              spam folder) and enter it here
            </p>
      ...
      <GreatJob show={show} onHide={handleClose} />
    </>
  );
};
