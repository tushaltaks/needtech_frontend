...
const Articles = () => {
  ...
  const getBlogs = async () => {
    const res = await GetFunction(
      `${baseURL}/getArticle?page=${page}&limit=${10}&category=${
        category || ""
      }`
    );
    // console.log('res', res)
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["blogs", page, category, pathname],
    queryFn: getBlogs,
    onError: (error) => {
      ...
      <div className="inner_head">
        <Container>
          <div className="inner_head_in">
            <h1 className="heading_type1">Articles</h1>
          </div>
        </Container>
      </div>
              ...
              {data?.[0] && (
                <Col md={6}>
                  <div className="article_sec_itm">
                    <div className="article_sec_img">
                      <img
                        onError={handleErrorImage}
                        src={
                          data?.[0]?.image
                            ? handleimageUrl(data?.[0]?.image)
                            : ""
                        }
                      />
                    </div>
                    <div className="article_sec_con">
                      <h2 className="heading_type2">
                        <Link to={`/article-detail/${data?.[0]?.slug}`}>
                          {data?.[0]?.title}
                        </Link>
                      </h2>
                      ...
                      <Link
                        to={`/article-detail/${data?.[0]?.slug}`}
                        className="readmore_btn"
                      >
                        Read article <img src={RightArrow} />
                      </Link>
                    </div>
                  </div>
                </Col>
                ...
                {data?.length > 0 &&
                  data?.slice(1, 4).map((item, index) => (
                    <div className="article_sec_itm" key={index}>
                      <div className="article_sec_img_main">
                        <div className="article_sec_img">
                          <img
                            onError={handleErrorImage}
                      ...
                      <div className="article_sec_con">
                        <h2 className="heading_type2">
                          <Link to={`/article-detail/${item.slug}`}>
                            {item?.title}
                          </Link>
                        </h2>
