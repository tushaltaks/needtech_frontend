...
const Marketplace = () => {
  ...
  const [innova, setInnovaRate] = useState(innovaRate ? innovaRate : "");
  ...
  const getBusinessList = async (pageone, categoryId, minPrice, maxPrice) => {
    const res = await GetFunction(
      `${baseURL}/businessList?userId=${token && userId ? userId : ""}&page=${
        pageone ? pageone : page
      }&limit=10&search=${search}&categoryId=${
  ...
  const addToWishList = async (id) => {
    ...
    if (res?.status == 200) {
      ...
      getBusinessList(categoryId, minPrice, maxPrice);
    } else {
      toast.dismiss();
      toast.error(res?.data?.message);
    }
  ...
  useEffect(() => {
    const innovaRate = searchParams.get("innovaRate");
    setInnovaRate(innovaRate ? innovaRate : "");
  }, [searchParams]);
  if (loader) {
    return <Loader />;
  }
      ...
      <div className="inner_head">
        <Container>
          <div className="inner_head_in">
            <h1 className="heading_type1">Startups Marketplace</h1>
          </div>
        </Container>
      </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (page > 1) {
                    setPage(1);
                    setTimeout(() => {
                      getBusinessList(1, categoryId, minPrice, maxPrice);
                    }, 100);
                  } else {
                    getBusinessList(page, categoryId, minPrice, maxPrice);
                  }
                }}
              >
                Search
                    ...
                    <div className="market_list_img">
                      <div className="market_list_img_in">
                        <img
                          ...
                          src={
                            val?.image ? handleimageUrl(val?.image) : Startupim1
                          }
                        />
                      </div>
                    </div>
                      ...
                      <div className="market_list_con_in">
                        ...
                        <div className="market_list_info">
                          ...
                          <p className={token ? "" : "locked_data"}>
                            {DOMPurify.sanitize(val?.shortDescription).replace(
                              /<[^>]+>/g,
                              ""
                            )}
                          </p>
                          {token ? (
                            ...
                            <div className={"locked_btn"}>
                              <Link to="/login" className="btn btn_outline">
                                <img src={Lokedic} /> Unlock Startup
                              </Link>
                            </div>
                          )}
